...
- `views/app/dm` is a full screen, not a dialog: a direct message opens at `/dm?user=<userId>`, which resolves (or creates) the DM room itself. The old `friend-chat-dialog` widget is gone; `FriendChatProvider` now only tracks the unread counter, keyed on the peer in the URL.
- `views/app/lobby` mirrors Discord's home: a header bar with Online / All / Servers tabs and an Add friend tab (a page, not a dialog), the friend list in the middle and an **Active now** panel on the right; the left column (`widgets/room/channels-panel`) holds the search trigger, the Friends entry and the direct-message list above the user bar.
- `views/app/server` reads `?id=&channel=&thread=` (query params, see [room-session.md](room-session.md) for why not path segments); `views/app/invite` reads `?code=`. `views/app/lobby` is now the Discord-style home: your servers plus friend activity, with no room browser.
