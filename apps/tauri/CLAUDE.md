...
bun android:build # build the APK + the Play AAB
```
...
Android setup: install Android Studio (SDK + NDK), copy `apps/tauri/.env.example` → `.env`, set `NDK_HOME` to your installed NDK folder, then `bun android:init`. See [docs/google-play/README.md](../../docs/google-play/README.md).
...
**FCM (push):** `google-services.json` lives in `android/` (committed). `scripts/setup-android-fcm.mjs` runs before every `android:*` build, copies it into `gen/android/app/` and applies the Google Services Gradle plugin — `gen/` is gitignored and must not be edited by hand. FCM is the only push transport; devices with no Google mobile services get no push.

**In-app updates on Android:** none, by design — Play forbids an app updating itself around the store. The Tauri updater is excluded at three layers: the dependency is behind `cfg(not(android))` in `Cargo.toml`, the plugin is registered under `#[cfg(desktop)]` in `lib.rs`, and `updater:default` only appears in `capabilities/desktop.json`. The client hook is gated on `isTauriDesktop()` on top of that.
